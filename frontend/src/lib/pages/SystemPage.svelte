...
<div class="h-full overflow-y-auto px-6 py-6">
  <div class="max-w-5xl mx-auto space-y-6">
    <div>
      ...
      <p class="text-sm text-muted">Live view of GPU and Ollama runtime state. Updates every 1.5s.</p>
    </div>
            ...
            <div class="grid grid-cols-3 gap-4">
              <div>
                ...
                <div class="text-lg font-mono text-body">
                  ...
                  <span class="text-muted text-sm"> / {(g.memory_total_mb / 1024).toFixed(0)} GB</span>
                </div>
                <div class="h-2 bg-surface-2 rounded overflow-hidden mt-1">
                  <div class="h-full bg-accent transition-all" style="width: {(g.memory_used_mb / g.memory_total_mb * 100).toFixed(1)}%"></div>
                </div>
              </div>
              <div>
                <div class="text-xs text-muted font-mono uppercase mb-1">utilization</div>
                <div class="text-lg font-mono text-body">{g.utilization_pct}<span class="text-muted text-sm">%</span></div>
                <div class="h-2 bg-surface-2 rounded overflow-hidden mt-1">
                  <div class="h-full bg-accent transition-all" style="width: {g.utilization_pct}%"></div>
                </div>
              </div>
              <div>
                <div class="text-xs text-muted font-mono uppercase mb-1">temperature</div>
                <div class="text-lg font-mono text-body">{g.temp_c}<span class="text-muted text-sm">°C</span></div>
              </div>
            </div>
          </div>
        {/each}
    ...
    <section class="bg-surface border border-border rounded-lg p-5">
      ...
      {#if loaded.length === 0}
        <div class="text-muted text-sm">No models currently loaded. Send a message to warm one up.</div>
      {:else}
        <div class="space-y-2">
          {#each loaded as m}
            <div class="bg-surface-2 border border-border rounded p-3">
              <div class="flex items-baseline justify-between gap-3 flex-wrap">
                ...
                <div class="text-xs text-muted font-mono">{fmtParams(m.details?.parameter_size)} · {m.details?.quantization_level}</div>
              </div>
